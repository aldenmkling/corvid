...
# Usage: bash scripts/runpod/analyze_h_residuals_runpod.sh
set -e
cd "$(dirname "$0")/../.."
ROOT="$(pwd)"
...
tar -czf "$TAR" \
    ...
    scripts/training \
    scripts/data_prep \
    scripts/testing/analyze_h_residuals.py \
...
$SSH "cd /workspace && tar -xzf payload.tar.gz && rm payload.tar.gz \
    ...
    && pip install -q -r scripts/runpod/requirements_farm_runpod.txt scikit-learn"

echo "=== running analysis ==="
$SSH "cd /workspace && source venv/bin/activate \
    && python scripts/testing/analyze_h_residuals.py --device cuda 2>&1"
