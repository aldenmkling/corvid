#!/usr/bin/env bash
# Run the play_065 tracking-dots rectified visual on a fresh RunPod pod.
# Usage: bash scripts/runpod/play_065_tracking_dots_runpod.sh
set -e
cd "$(dirname "$0")/../.."
ROOT="$(pwd)"
...
# Read SSH info from the saved state.
...
TAR=/tmp/play_065_dots_payload.tar.gz
tar -czf "$TAR" \
...
$SCP "$TARGET:/workspace/output/play_065_visuals/tracking_dots.mp4" "$ROOT/output/play_065_visuals/"

echo "=== terminating pod ==="
.venv/bin/python scripts/runpod/launch_runpod.py --terminate

echo "=== done ==="
ls -la "$ROOT/output/play_065_visuals/tracking_dots.mp4"
