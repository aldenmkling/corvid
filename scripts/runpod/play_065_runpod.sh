#!/usr/bin/env bash
# Run the play_065 3-visual generator on a fresh RunPod pod.
# Usage: bash scripts/runpod/play_065_runpod.sh
set -e
cd "$(dirname "$0")/../.."
ROOT="$(pwd)"
...
TAR=/tmp/play_065_payload.tar.gz
tar -czf "$TAR" \
...
$SSH "cd /workspace && tar -xzf payload.tar.gz && rm payload.tar.gz \
    ...
    && pip install -q -r scripts/runpod/requirements_farm_runpod.txt rfdetr"
