...
echo "=== creating pod ==="
.venv/bin/python scripts/runpod/launch_runpod.py --create-only --training-type unet --gpu-type "NVIDIA GeForce RTX 5090" --cloud-type ALL --disk-size 40 --create-retries 6 --min-upload 50
...
tar -czf "$TAR" \
    ...
    scripts/training \
    scripts/data_prep \
    scripts/testing/compare_ngs_play_065.py \
...
$SSH "cd /workspace && tar -xzf payload.tar.gz && rm payload.tar.gz \
    ...
    && pip install -q -r scripts/runpod/requirements_farm_runpod.txt rfdetr scikit-learn pandas"

echo "=== running NGS compare ==="
$SSH "cd /workspace && source venv/bin/activate \
    && python scripts/testing/compare_ngs_play_065.py --device cuda 2>&1"
