...
echo "=== creating pod ==="
.venv/bin/python scripts/aux/runpod/launch_runpod.py --create-only --training-type unet --gpu-type "NVIDIA GeForce RTX 5090" --cloud-type ALL --disk-size 40 --create-retries 6 --min-upload 5
...
$SSH "cd /workspace && tar -xzf payload.tar.gz && rm payload.tar.gz \
    ...
    && pip install -q -r scripts/aux/runpod/requirements_farm_runpod.txt rfdetr scikit-learn"
