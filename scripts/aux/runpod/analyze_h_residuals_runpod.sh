...
# Usage: bash scripts/aux/runpod/analyze_h_residuals_runpod.sh
set -e
cd "$(dirname "$0")/../../.."
ROOT="$(pwd)"
...
tar -czf "$TAR" \
    ...
    scripts/aux/diagnostics/analyze_h_residuals.py \
...
$SSH "cd /workspace && source venv/bin/activate \
    && python scripts/aux/diagnostics/analyze_h_residuals.py --device cuda 2>&1"
