#!/usr/bin/env python3
"""
Hash-pair based yard line identification.

Pipeline:
  1. Detect hashes (high conf, threshold 0.4) and sidelines (lower conf, 0.3).
  2. Split hashes into near/far rows by y-pixel.
  3. Pair each far hash with a near hash on the same yard line (x-proximity + vertical gap).
  4. For each pair, project the yard line up to the far sideline and look for a matching detection.
  5. Assign grid positions by horizontal spacing of the paired hashes (median spacing = 5 yards).

No homography yet — just produces a structured list of yard line groups so we
can visually verify the grouping and grid assignment.
"""
...
DEFAULT_FRAME = os.path.join(PROJECT_ROOT, "output", "camera_model_test", "real_kickoff_raw.jpg")
...
HASH_ROW_SPAN_YARDS = 29.75 - 23.583      # near→far hash = 6.17 yards
HASH_TO_FAR_SIDELINE_YARDS = 53.33 - 29.75  # far hash → far sideline = 23.58 yards
HASH_TO_NEAR_SIDELINE_YARDS = 23.583       # near hash → near sideline
